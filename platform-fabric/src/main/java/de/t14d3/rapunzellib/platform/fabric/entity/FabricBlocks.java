...
public final class FabricBlocks implements Blocks {
    // LRU cache for block wrappers using primitive longs (zero allocation lookups)
    private static final int CACHE_SIZE = 10_000;
    private final Long2ObjectLinkedOpenHashMap<FabricBlock> blockCache = new Long2ObjectLinkedOpenHashMap<>(16, 0.75f);
    ...
    public @NotNull Optional<RBlockData> wrapData(@NotNull Object nativeBlockData) {
        if (nativeBlockData instanceof BlockState state) return Optional.of(new FabricBlockData(state));
        if (nativeBlockData instanceof Block block) return Optional.of(new FabricBlockData(block.defaultBlockState()));
        return Optional.empty();
    }
    ...
    public @NotNull RBlock at(@NotNull RWorld world, @NotNull RBlockPos pos) {
        ...
        synchronized (blockCache) {
            FabricBlock cached = blockCache.getAndMoveToLast(key);
            if (cached != null) {
                return cached;
            }
            ...
            FabricBlock newBlock = new FabricBlock(level, new BlockPos(pos.x(), pos.y(), pos.z()));
    ...
    public @NotNull Optional<RBlockData> parseData(@NotNull String value) {
        ...
        return Optional.of(new FabricBlockData(block.defaultBlockState()));
    }
}
